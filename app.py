...
st.markdown(f"""<div class="thresh-bar">
  ...
  <span>📦 Rejection Rate: RUNNING → rejected ÷ produced × 100% &nbsp;·&nbsp; FAULT → 100% (defined) &nbsp;·&nbsp; <b>Recalculates with filters</b></span>
...
| Metric | Formula | Note |
|--------|---------|------|
| **Availability (A)** | Running hours ÷ (Running + Fault + Idle hours) | Fraction of time actively running |
| **Performance (P)** | Actual output ÷ (Peak hourly rate × Running hours) | How close to max possible output |
| **Quality (Q)** | (Produced − Rejected) ÷ Produced | Good unit rate during running |
| **OEE** | A × P × Q × 100% | World-class target: ≥ 85% (SEMI E10) |
| **Plant Uptime** | Running readings ÷ Total readings × 100% | — |
| **Yield Rate** | (Produced − Rejected) ÷ Produced × 100% | — |
| **Rejection Rate (RUNNING/IDLE)** | Rejected ÷ Produced × 100% | From raw data: rejected_units / produced_units |
| **Rejection Rate (FAULT)** | **100%** — by definition | Raw data shows 0 produced & 0 rejected during FAULT. Defined as 100% to reflect total production loss. This is a dashboard design choice, not a raw sensor value. |
| **⚠️ All rates recalculate with filters** | Changing date, machine, or shift changes the sample and therefore all rates | — |
...
| **Current threshold ({cur_thresh:.0f} A)** | Specified in CA2 dataset · ≈ 2× nominal load (~30 A) · Consistent with overcurrent fault detection practice | — |
| **Vibration threshold ({vib_thresh:.0f} mm/s)** | Specified in CA2 dataset · Consistent with ISO 10816/20816 — above 10 mm/s is "unsatisfactory" for industrial motors | — |
""")
